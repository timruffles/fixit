...
func (vq *VoteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Vote, error) {
	...
	if vq.withPost != nil || vq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := vq.withPost; query != nil {
		if err := vq.loadPost(ctx, query, nodes, nil,
			func(n *Vote, e *Post) { n.Edges.Post = e }); err != nil {
			return nil, err
		}
	}
...
func (vq *VoteQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Vote, init func(*Vote), assign func(*Vote, *Post)) error {
	...
	for i := range nodes {
		if nodes[i].vote_post == nil {
			continue
		}
		fk := *nodes[i].vote_post
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "vote_post" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (vq *VoteQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Vote, init func(*Vote), assign func(*Vote, *User)) error {
	...
	nodeids := make(map[uuid.UUID][]*Vote)
	for i := range nodes {
		if nodes[i].vote_user == nil {
			continue
		}
		fk := *nodes[i].vote_user
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "vote_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
